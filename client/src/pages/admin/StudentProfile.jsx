...
const InfoItem = ({ label, value, monospace, highlight }) => (
    <div style={{ paddingBottom: '0.5rem', borderBottom: '1px solid #f4f4f5' }}>
        <div style={{ fontSize: '0.7rem', color: '#71717a', textTransform: 'uppercase', fontWeight: '600' }}>{label}</div>
        <div style={{
            fontSize: '0.85rem',
            color: highlight ? '#18181b' : '#3f3f46',
...
const ActionModal = ({ title, description, confirmText, confirmColor, onConfirm, onCancel }) => (
    <div style={{
        ...
        <div style={{ background: 'white', padding: '2rem', borderRadius: '12px', width: '400px', maxWidth: '90%' }}>
            ...
            <p style={{ color: '#71717a', marginBottom: '1.5rem', lineHeight: 1.5 }}>{description}</p>
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
                <button onClick={onCancel} style={{ padding: '0.5rem 1rem', borderRadius: '6px', border: '1px solid #e4e4e7', background: 'white', cursor: 'pointer' }}>Cancel</button>
...
const HeaderSection = ({ user }) => (
    <div style={{
        ...
        border: '1px solid #e4e4e7', position: 'relative', overflow: 'hidden'
    }}>
        <div style={{
            position: 'absolute', top: '10px', right: '10px',
            background: '#f4f4f5', color: '#a1a1aa', fontSize: '0.65rem', fontWeight: 'bold',
        ...
        <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.5rem' }}>
                <h1 style={{ fontSize: '1.75rem', fontWeight: '700', color: '#18181b', margin: 0 }}>{user.name}</h1>
                <span style={{
            ...
            <div style={{ display: 'flex', gap: '2rem', color: '#71717a', fontSize: '0.95rem' }}>
...
const AttendanceSection = ({ mockStats, mockAttendanceHistory }) => (
    <div style={cardStyle}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
            <h3 style={sectionHeaderStyle}><Clock size={20} /> Attendance</h3>
            <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: '2rem', fontWeight: '800', color: '#18181b', lineHeight: 1 }}>{mockStats.attendancePercentage}%</div>
                <div style={{ fontSize: '0.75rem', color: '#71717a', fontWeight: '600' }}>OVERALL ATTENDANCE</div>
            </div>
        </div>
        ...
        <div style={{ borderTop: '1px solid #e4e4e7', paddingTop: '1rem' }}>
            <h4 style={{ fontSize: '0.85rem', fontWeight: '600', color: '#52525b', marginBottom: '1rem', textTransform: 'uppercase' }}>Recent Activity</h4>
            <div style={{ maxHeight: '200px', overflowY: 'auto' }}>
                {mockAttendanceHistory.map((record) => (
                    <div key={record.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 0', borderBottom: '1px solid #f4f4f5' }}>
                        <div>
                            <div style={{ fontSize: '0.9rem', fontWeight: '500', color: '#18181b' }}>{record.subject}</div>
                            <div style={{ fontSize: '0.75rem', color: '#71717a' }}>{record.date}, {record.time}</div>
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                            ...
                            <span style={{ fontSize: '0.7rem', color: '#a1a1aa' }}>{record.method}</span>
                        </div>
                    </div>
                ))}
            </div>
...
const SecuritySection = ({ user, mockSecurity, setActiveModal }) => (
    <div style={cardStyle}>
        <h3 style={sectionHeaderStyle}><Shield size={20} /> Account & Security</h3>

        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem', background: '#fafafa', padding: '1rem', borderRadius: '8px' }}>
            <div>
                <div style={{ fontSize: '0.75rem', color: '#71717a', textTransform: 'uppercase', fontWeight: '600' }}>Active Session</div>
                <div style={{ fontSize: '0.95rem', fontWeight: '600', color: mockSecurity.activeSession ? '#18181b' : '#71717a' }}>
                    {mockSecurity.activeSession ? 'Online Now' : 'Offline'}
                </div>
            </div>
            <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: '0.75rem', color: '#71717a', textTransform: 'uppercase', fontWeight: '600' }}>Last Login</div>
                <div style={{ fontSize: '0.9rem', color: '#18181b' }}>{mockSecurity.lastLogin}</div>
            </div>
        </div>
...
const PlacementSection = ({ mockStats }) => (
    <div style={cardStyle}>
        <h3 style={sectionHeaderStyle}><Briefcase size={20} /> Placement Eligibility</h3>
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', marginBottom: '1.5rem' }}>
            <div style={{ flex: 1 }}>
                <div style={{ fontSize: '0.75rem', color: '#71717a', textTransform: 'uppercase', fontWeight: '600', marginBottom: '4px' }}>CGPA</div>
                <div style={{ fontSize: '1.5rem', fontWeight: '700', color: '#3f3f46' }}>{mockStats.cgpa}</div>
            </div>
            <div style={{ flex: 1 }}>
                <div style={{ fontSize: '0.75rem', color: '#71717a', textTransform: 'uppercase', fontWeight: '600', marginBottom: '4px' }}>Status</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div style={{ background: '#fafafa', padding: '1rem', borderRadius: '8px', textAlign: 'center' }}>
                <div style={{ fontSize: '1.25rem', fontWeight: '700', color: '#18181b' }}>{mockStats.eligibleDrives}</div>
                <div style={{ fontSize: '0.7rem', color: '#71717a', marginTop: '4px' }}>Eligible Drives</div>
            </div>
            <div style={{ background: '#fafafa', padding: '1rem', borderRadius: '8px', textAlign: 'center' }}>
                <div style={{ fontSize: '1.25rem', fontWeight: '700', color: '#18181b' }}>{mockStats.appliedDrives}</div>
                <div style={{ fontSize: '0.7rem', color: '#71717a', marginTop: '4px' }}>Applied</div>
            </div>
        </div>
    </div>
);

const ViolationSection = ({ mockStats }) => (
    <div style={cardStyle}>
        ...
        {mockStats.proxyAttempts > 0 ? (
            ...
            <div style={{ color: '#71717a', fontStyle: 'italic', padding: '1rem', textAlign: 'center' }}>No violations detected.</div>
        )}
    </div>
);

const ActivityLogSection = ({ isLogExpanded, setIsLogExpanded, mockActivityLog }) => (
    <div style={{ ...cardStyle }}>
        <div
        ...
        >
            ...
            {isLogExpanded ? <ChevronUp size={20} color="#71717a" /> : <ChevronDown size={20} color="#71717a" />}
        </div>

        {isLogExpanded && (
            <div style={{ marginTop: '1.5rem', borderTop: '1px solid #e4e4e7', paddingTop: '1rem' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
                    ...
                    <tbody>
                        {mockActivityLog.map(log => (
                            <tr key={log.id} style={{ borderTop: '1px solid #fafafa' }}>
                                <td style={{ padding: '0.75rem', color: '#3f3f46' }}>{log.admin}</td>
                                <td style={{ padding: '0.75rem', color: '#3f3f46' }}>{log.action}</td>
                                <td style={{ padding: '0.75rem', color: '#71717a', fontSize: '0.85rem' }}>{log.time}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
...
export default function StudentProfile() {
    ...
    if (loading) return (
        <AdminLayout>
            <div className="flex items-center justify-center h-64 text-zinc-500">Loading profile...</div>
        </AdminLayout>
    );
        ...
        <AdminLayout title="Student Profile" backUrl="/admin/users">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr 300px', gap: '1.5rem', alignItems: 'start' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    <SecuritySection user={user} mockSecurity={mockSecurity} setActiveModal={setActiveModal} />
                    <PlacementSection mockStats={mockStats} />
                </div>

                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    <AttendanceSection mockStats={mockStats} mockAttendanceHistory={mockAttendanceHistory} />
                    <ActivityLogSection isLogExpanded={isLogExpanded} setIsLogExpanded={setIsLogExpanded} mockActivityLog={mockActivityLog} />
                </div>

                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                    <ViolationSection mockStats={mockStats} />
                    <div style={{ ...cardStyle, minHeight: '150px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#a1a1aa', fontSize: '0.9rem', flexDirection: 'column', gap: '8px' }}>
                        <Info size={24} />
